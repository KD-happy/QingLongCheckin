            ...
            try:
                ...
                if login_token == 0:
                        msg = [
                            ...
                            {"name": "修改状态", "value": f"登陆失败"},
                        ]
                else:
                    t = int(time.time()) #self.get_time()
                    ...
                    msg = [
                        {"name": "帐号信息", "value": f"{phone[:4]}****{phone[-4:]}"},
                        {"name": "修改状态", "value": f"{response['message']}"},
...
if __name__ == '__main__':
    ...
    Cookies = config.get('XMYD')
    if Cookies != None:
        if Cookies.get('cookies') != None:
            xmyd = XMYD(Cookies['cookies'])
            sio = xmyd.SignIn()
            print(f'\n{sio.getvalue()}')
            if Cookies.get('send') != None and Cookies['send'] == 1:
                send('小米运动', sio.getvalue())
            else:
                print('推送失败: 关闭了推送 or send配置问题')
        else:
            print('配置文件 小米运动 没有 "cookies"')
            sys.exit()
    else:
        print('配置文件没有 小米运动')
